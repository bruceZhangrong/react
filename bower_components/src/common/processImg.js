...
define(['common/exif'],function(EXIF){
	return compressImg = function(inputId,afterWidth,callback){
		var scale = afterWidth;
	        ...
	        p.onload = function(){
	            ...
	            if(upImgWidth<10||upImgWidth<10){
	              ...
	              return false;
	            	...
	            	// 获取压缩后生成的img对象
            		viewImg.src = convertCanvasToImage(hidCanvas,scale).src;
            		self.value = "";
            		// 此处将得到的图片数据回调
	...
	//canvas转图像
	function convertCanvasToImage(canvas,scale) {
	  var image = new Image();
